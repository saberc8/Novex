...
- MCP Gateway 2.0 stdio process supervisor, OAuth token exchange/callback integration, persisted sessions, and deployed external MCP server smoke hardening.
...
| MCP | `codex-rs/codex-mcp`, `rmcp-client` | `crates/novex-mcp` | adapt | slice-6 implemented | Tenant-governed registration, discovery, model-visible tool mapping, audit path, mock/dry-run invocation, Streamable HTTP request/response contract, gated backend live HTTP dispatch, offline local live-server smoke coverage, stdio launch/lifecycle contract, and OAuth authorization-code/PKCE plan contract exist; stdio process supervisor, OAuth token exchange/callback integration, persisted sessions, and deployed external MCP server smoke coverage remain follow-up |
