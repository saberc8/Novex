...
- MCP Gateway 2.0 stdio lifecycle, OAuth, persisted sessions, and deployed external MCP server smoke hardening.
...
| MCP | `codex-rs/codex-mcp`, `rmcp-client` | `crates/novex-mcp` | adapt | slice-4 implemented | Tenant-governed registration, discovery, model-visible tool mapping, audit path, mock/dry-run invocation, Streamable HTTP request/response contract, gated backend live HTTP dispatch, and offline local live-server smoke coverage exist; stdio lifecycle, OAuth, persisted sessions, and deployed external MCP server smoke coverage remain follow-up |
