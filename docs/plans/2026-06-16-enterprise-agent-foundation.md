...
- MCP Gateway 2.0 stdio process supervisor, OAuth, persisted sessions, and deployed external MCP server smoke hardening.
...
| MCP | `codex-rs/codex-mcp`, `rmcp-client` | `crates/novex-mcp` | adapt | slice-5 implemented | Tenant-governed registration, discovery, model-visible tool mapping, audit path, mock/dry-run invocation, Streamable HTTP request/response contract, gated backend live HTTP dispatch, offline local live-server smoke coverage, and stdio launch/lifecycle contract exist; stdio process supervisor, OAuth, persisted sessions, and deployed external MCP server smoke coverage remain follow-up |
