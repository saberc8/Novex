...
设计原则是：尽可能以根目录 `codex/` 源码为上游工程基准迁移 Agent 基建，不凭空手写玩具版运行时。Novex 保留企业控制面、租户、权限、模型路由、PostgreSQL 资产和统一后台交付；Codex 提供 Agent Runtime、tool protocol、event stream、approval、rollout/trace、sandbox/exec policy、MCP 等核心基建参考。
