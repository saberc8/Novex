...
| Rollout trace | `codex-rs/rollout*` | `crates/novex-trace` | adapt | slice-9 implemented | TraceBundle, replay API, `ai_rollout`, eval case capture, `trace_replay` eval gate, runtime span preservation for retrieval/action selection/context compaction/cancellation, model inference spans for route/provider/model/latency/usage/cost, provider error spans, route-policy provider retry spans, route fallback lifecycle attempts, process-local route circuit breaker cooldown attempts, bounded multi-hop provider lifecycle attempts, and eval runtime/model/retry/fallback/circuit-breaker tags are in place; persisted cross-process breaker state remains next |
