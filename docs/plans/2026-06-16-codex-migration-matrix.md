...
| Parallel tools | `codex-rs/core/src/tools/parallel.rs` | `crates/novex-tools` + backend model loop | adapt | slice-3 implemented | Shared/exclusive lock policy, cancellation-wait metadata, deterministic batch planning, parsed tool-call batches, backend batch event visibility, and true parallel tool I/O are in place; cancellation propagation remains next |
...
| Parallel tools | `novex-tools` exposes shared/exclusive locks, cancellation-wait metadata, deterministic batch planning, and backend `ActionSelected` `concurrencyPolicy`, `batchExecutionMode`, `toolCallBatch`, plus parallel tool I/O with serial audit/step persistence | `cargo test -p novex-tools --offline && cargo test -p backend-rust parallel_tool --offline && cargo test -p backend-rust model_loop --offline` |
