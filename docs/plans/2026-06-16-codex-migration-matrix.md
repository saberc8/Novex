...
| Runtime loop | `codex-rs/core/src/session/turn.rs`, `codex-rs/core/src/compact*` | `crates/novex-agent-runtime` | adapt | slice-4 implemented | Budget-bounded multi-turn model loop maps model output, single/batch tool calls, observations, context compaction, and final answer to Run Graph events; remote/model compaction remains next |
...
| Parallel tools | `codex-rs/core/src/tools/parallel.rs` | `crates/novex-tools` | adapt | slice-2 implemented | Shared/exclusive lock policy, cancellation-wait metadata, deterministic batch planning, parsed tool-call batches, and backend batch event visibility are in place; true async multi-tool execution remains next |
...
| Runtime loop POC | `runtimeMode=model_loop` uses configured `runtime.llm.code_agent`, parses single and batch model tool-call output, executes budget-bounded tool calls, records observations, compacts accumulated context after tool observations, and keeps sampling until final answer, approval pause, or budget stop | `cargo test -p novex-agent-runtime --offline && cargo test -p backend-rust model_loop --offline` |
...
| Parallel tools | `novex-tools` exposes shared/exclusive locks, cancellation-wait metadata, deterministic batch planning, and backend `ActionSelected` `concurrencyPolicy`, `batchExecutionMode`, and `toolCallBatch` visibility | `cargo test -p novex-tools --offline && cargo test -p backend-rust model_loop --offline` |
