...
| Rollout trace | `codex-rs/rollout*` | `crates/novex-trace` | adapt | slice-5 implemented | TraceBundle, replay API, `ai_rollout`, eval case capture, `trace_replay` eval gate, runtime span preservation for retrieval/action selection/context compaction/cancellation, model inference spans for route/provider/model/latency/usage/cost, provider error spans, and eval runtime tags are in place; provider-native retry/fallback and nested provider lifecycle spans remain next |
