...
| Parallel tools | `codex-rs/core/src/tools/parallel.rs` | `crates/novex-tools` | adapt | slice-1 implemented | Shared/exclusive lock policy, cancellation-wait metadata, deterministic batch planning, and backend event visibility are in place; true async multi-tool execution remains next |
...
| Parallel tools | `novex-tools` exposes shared/exclusive locks, cancellation-wait metadata, deterministic batch planning, and backend `ActionSelected` `concurrencyPolicy` visibility | `cargo test -p novex-tools --offline && cargo test -p backend-rust model_loop --offline` |
