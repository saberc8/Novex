...
describe("eval api wrappers", () => {
  ...
  it("uses eval dataset, case, run, report, and result endpoints", async () => {
    ...
    expect(fetchMock.mock.calls[1]?.[0]).toBe(
      "http://localhost:4398/ai/evals/datasets/3400001/cases?page=1&size=100&targetKind=rag"
    );
    expect(fetchMock.mock.calls[2]?.[0]).toBe("http://localhost:4398/ai/evals/runs");
    expect(fetchMock.mock.calls[2]?.[1]).toMatchObject({
      method: "POST",
      body: JSON.stringify({ datasetCode: "training_regression" })
    });
    expect(fetchMock.mock.calls[3]?.[0]).toBe(
      "http://localhost:4398/ai/evals/runs?page=1&size=10&datasetCode=training_regression"
    );
    expect(fetchMock.mock.calls[4]?.[0]).toBe("http://localhost:4398/ai/evals/runs/42");
    expect(fetchMock.mock.calls[5]?.[0]).toBe(
      "http://localhost:4398/ai/evals/runs/42/results?page=1&size=100"
    );
  });
});
